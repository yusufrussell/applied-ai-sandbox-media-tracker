...
Planning happened in Claude.ai chat (no commit pressure, just decisions on paper). Execution happened in Claude Code terminal sessions — it read CLAUDE.md automatically and followed the factory pattern, type hint conventions, and "no database" constraint without being reminded each time. Copilot handled small inline completions while reviewing Claude's diffs in VS Code — accepting `.get("starred", False)` defaults and one-line docstrings that weren't worth a full agent round-trip.

The clearest moment one tool outperformed another: Claude Code caught the `url_for('add')` crash in Task 4 before I did. Chat's design operated at the route level and never mentioned that templates resolve `url_for` at render time — Claude Code was in the code and flagged it immediately when the test client returned a 500.

The moment I switched tools mid-task: during Task 7, after Claude Code implemented the detail page, I switched to Copilot in VS Code to rename `idx` → `item_id` across the route and template. That's a 2-second tab-completion, not a full agent brief — using Claude Code for it would have been slower.

---

## Reflection

*(See Step 7 for the full 300-word reflection — to be added after manual end-to-end verification.)*
